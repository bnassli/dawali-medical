...
ALTER TABLE "clinical_field_definitions" ADD CONSTRAINT "clinical_field_definitions_section_id_clinical_sections_id_fk" FOREIGN KEY ("section_id") REFERENCES "public"."clinical_sections"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "clinical_field_definitions_section_code_idx" ON "clinical_field_definitions" USING btree ("section_id","code");--> statement-breakpoint
CREATE UNIQUE INDEX "clinical_options_list_id_lower_label_idx" ON "clinical_options" USING btree ("list_id",lower("label"));